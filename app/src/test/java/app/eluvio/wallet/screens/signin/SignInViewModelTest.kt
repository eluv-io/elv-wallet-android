...
class SignInViewModelTest {
    ...
    private val metamaskActivationStore = mockk<MetamaskActivationStore> {
        every { observeMetamaskActivationData(any(), any()) } returns Flowable.just(
            MetamaskActivationData(
                "activation1",
                "passcode",
                "http://url",
                999
    ...
    fun `on activation complete should prefetch properties and navigate`() {
        ...
        // WHEN
        every { metamaskActivationStore.checkToken(any()) } returns Maybe.just(csatSuccess())
        vm.onResume()
