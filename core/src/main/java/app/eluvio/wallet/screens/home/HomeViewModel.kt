...
class HomeViewModel @Inject constructor(
    ...
    private val navArgs = savedStateHandle.toRoute<DeeplinkArgs>()
        ...
        Maybe.fromCallable { navArgs.toDeeplinkRequest() }
            ...
            .subscribeBy(
                ...
                onComplete = {
                    ...
                    navigateTo(NavTarget.Dashboard.asNewRoot())
                },
                onError = { }
            )
            .addTo(disposables)
    ...
    private fun DeeplinkRequestEntity.toVideoPlayerTarget(): NavTarget {
        //TODO: fix this hack once we figure out what we actually want to do with ://play actions
        return NavTarget.VideoPlayer(
...
fun DeeplinkRequestEntity.toNftClaimTarget(): NavTarget? {
    return NavTarget.NftClaim(
