    ...
    Box(
        ...
        Row(modifier = Modifier.padding(top = topPadding)) {
            ...
            Column(
                ...
                if (hasFilterRow) {
                    val filterAttribute = item.filterAttribute!!
                    FilterSelectorRow(
                        selectedValue = selectedFilter?.second,
                        attributeValues = filterAttribute.values.map { it.value }
                            .toImmutableList(),
                        onValueSelected = { tag ->
                            selectedFilter = tag?.let { filterAttribute.id to it }
                        },
                        modifier = Modifier
                    ...
                    val viewAllNavigationEvent = item.viewAllNavigationEvent
                    if (!hasTitleRow && viewAllNavigationEvent != null) {
                        ViewAllButton(
                            viewAllNavigationEvent,
